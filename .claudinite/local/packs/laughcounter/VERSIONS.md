...
| Date | Task | Change |
|---|---|---|
| 2026-08-30 | `prose-to-checks-sweep` | Added: `laughcounter/single-version-source` — a regression guard for the "ship a version the running app can state" prose (`RULES.md`, #74/#75), banning a hand-written `v<version> (<build>)` literal outside `AppDelegate.swift`. The prose itself stays: it carries the release-workflow mechanics and incident history the check doesn't. |
